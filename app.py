...
PHOTOAGE_ENABLE_STAT = os.getenv('PHOTOAGE_ENABLE_STAT', 'false') == 'true'

app = flask.Flask('baby-everyday', static_url_path=f'/static/{SECRET_PATH}')


def detect_birthday(dir_path, file_names, photoage_enable_stat):
    get_date_time_options = {}
    if photoage_enable_stat:
        get_date_time_options['methods'] = ('exif', 'stat')
...
def generate_mapping(side, birthday, photoage_offset, photoage_enable_stat):
    dir_path = os.path.join('static', 'photos', side)
    file_names = os.listdir(dir_path)

    calculate_days_options = {}
    if birthday is None:
        birthday = detect_birthday(dir_path, file_names, photoage_enable_stat)
    if birthday:
    ...
    if photoage_enable_stat:
        calculate_days_options['methods'] = ('exif', 'stat')
...
left_mapping = generate_mapping('left', LEFT_BIRTHDAY, PHOTOAGE_OFFSET, PHOTOAGE_ENABLE_STAT)
right_mapping = generate_mapping('right', RIGHT_BIRTHDAY, PHOTOAGE_OFFSET, PHOTOAGE_ENABLE_STAT)
